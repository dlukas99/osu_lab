...
KNN_model_k5.fit(X_train_n, y_train)

# K=5
y_train_knn_k5 = KNN_model_k5.predict(X_train_n)
y_test_knn_k5 = KNN_model_k5.predict(X_test_n)

print("K=5")
print("Tocnost train: " + "{:0.3f}".format(accuracy_score(y_train, y_train_knn_k5)))
print("Tocnost test: " + "{:0.3f}".format(accuracy_score(y_test, y_test_knn_k5)))

#  K=5
plot_decision_regions(X_train_n, y_train, classifier=KNN_model_k5)
plt.xlabel('x1')
plt.ylabel('x_2')
plt.legend(loc='upper left')
plt.title("KNN (K=5) - Tocnost: " + "{:0.3f}".format(accuracy_score(y_train, y_train_knn_k5)))
plt.tight_layout()
plt.show()
